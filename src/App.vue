<template>
  <div class="container w-md-50">
    ...
    <!-- Gauges -->
    <div class="mt-2 mb-3" style="z-index:98;">
      <!-- Enemy Gauge -->
      <AttackGauge 
    ...
    <!-- Player Actions -->
    <div class="abilityCard-container position-relative">
...
import SocketioService from './assets/SocketService';
...
export default {
  ...
  created: function() {
    ...
    SocketioService.bind('turnEvents', async (data) => {
      console.log(JSON.stringify(data, null, 2));
      for(const event of data){
        if(event.target && event.target.includes('.')) {
          let split = event.target.split('.')
          let target = split[0];
          let method = split[1];
          switch(method) {
            case 'attack':
              await this.battle[target].addAttack(event.modifier);
          }
        }
      }
    })
  ...
  mounted() {
    ...
    body.style.background = `linear-gradient(0deg, black, #00000090), url(${require(`@/assets/backgrounds/Hills.png`)})`;
  ...
  methods: {
    ...
    handlePlayerAction(action, cardId=null) {
      ...
      SocketioService.emit('handlePlayerAction', {action, cardId});
    },
  }
};
